...
description: Use PARA to manage owner-scoped LinkedIn and Reddit research, consume persisted results and drafts, and hand reviewed content to the separately authenticated publishing workflow.
license: Proprietary
...
Use this skill for LinkedIn and Reddit research and its human-review handoff. The
public API controls registration intent, collection lifecycle, result delivery,
and advisory drafts. Collection is asynchronous and runs only through the
already-paired real Chrome extension. Reviewed publishing uses the dashboard and
paired participation extension, not the public API key.

## Canonical context

- One-page integration guide: `https://para-linkedin-api.mintlify.app/agent.md`
...
Read the one-page integration guide before any mutation. OpenAPI is authoritative
for methods, paths, schemas, and webhook contracts. Do not crawl legacy guide
pages unless resolving an old link.
...
- confirm an edited draft for publishing
- cancel a pending publishing item

Unsupported:

- direct LinkedIn or Reddit scraping
- Playwright, Chromium, or fixture collection as a production substitute
- unreviewed or unapproved draft submission
- using a public API key on dashboard publishing endpoints
- calling extension claim or completion endpoints without a paired device token
- real-world identity resolution for Reddit authors

## Human review and approved publishing

1. Collection and generation persist each selected result and any ready draft.
2. Ready drafts are automatically available in the human review queue. This is
   not a platform action.
3. A reviewer may edit or reject the draft.
4. Explicit `Confirm to send` creates a durable publishing queue snapshot.
5. The assigned extension later executes only approved snapshots and reports
   queued, claimed, posted, retrying, failed, or cancelled state.

The public `/v1/*` contract can return drafts but cannot confirm them for
publishing. LinkedIn and Reddit publishing-queue mutations are dashboard-session
operations under `/api/*`. If the handoff provides only `PARA_API_KEY`, complete
the research workflow and report that the draft is ready for dashboard review.
...
- reply drafts are reviewable; confirmation is dashboard-only and approved queue
  execution is extension-only.
